 ...
 */
app.controller( 'TableController', ['$scope', '$rootScope', '$http', '$routeParams', '$timeout', 'sounds', '$window',
function( $scope, $rootScope, $http, $routeParams, $timeout, sounds, $window ) {
	var seat = null;
	$scope.table = {};
	$scope.notifications = [{},{},{},{},{},{},{},{},{},{}];
	...
	$scope.updateBetBar = function() {
			$scope.betAmount = parseInt($scope.betAmountRange);
	};
	...
	function focusBet(){
		var element = $window.document.getElementById('bet-input');
		if(element){
			element.focus();
			element.select();
		}
	...
	// When the user recibe help when is the turn
	socket.on( 'actRecibeHelp', function(data) {
		$scope.help = data;

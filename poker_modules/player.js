var typesPlayer = ['Card', 'Rock', 'Mouse', 'Monkey', 'Telephone', 'ABC', 'Lion',
        'Man', 'Whale', 'Eagle', 'Crow', 'ABC_FO', 'Dice', 'Fish', 'Snake'];
 ...
 * @param string types (cualquiera de typesPlayer^ o puede ser: 'PRO' o 'USER')
 * PRO va ser un bot pero va responder como analizerControlado osea el mejor
 * USER un usuario del sistema no se lo determina
 * PRO y USER van a tener como typeN el valor de 5 (ABC) porque el server no lee otros numeros
 */
var Player = function( socket, name, chips, type ) {

  // PRO es el tipo para los humanos y los pro, prueba
  type = type === undefined ? 'PRO' : type;
	...
	this.public = {
        ...
        // "Man", "Whale", "Eagle", "Crow", "ABC_FO", "Dice", "Fish", "Snake"
        type: type,
        typeN: typesPlayer.indexOf(type) > -1 ? typesPlayer.indexOf(type) : 5
	};
 ...
 * getNtype player
 */
Player.prototype.getNtype = function(type) {
    var types = ["Card", "Rock", "Mouse", "Monkey", "Telephone", "ABC", "Lion",
...
Player.prototype.sitOut = function() {
    if( this.sittingOnTable !== false ) {
        ...
        console.log("***** el jugador: " + this.public.name + " ha sido eliminado :(");
    }
};

/**
 * Used to know if the player is a Bot
 */

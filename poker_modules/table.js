...
var urlBase = process.env.ENV === 'prod' ? process.env.URL_BOTS_PROD : process.env.URL_BOTS_LOCAL;
...
var Table = function( id, name, eventEmitter, seatsCount, bigBlind, smallBlind, maxBuyIn, minBuyIn, privateTable ) {
	...
	this.pot = new Pot;
	// para no enviar el mismo en caso de fail en la tabla
	this.lastMessage = '';
	// realiza el proceso de las blinds sin preuntar al usuario
	this.autoBlinds = true;
	// cuando un bot es eliminado recompra automaticamente
	this.autoRebuyBots = true;
	// cuando estan dos players inicia la mano automaticamente
	this.autoBeginWhen2 = true;
	// cuando esta activo sigue a la siguiente fase automaticamente
	this.autoSigPhase = true;
	// carga humanos de la tabla asi no se hayan sentado a jugar
	this.observers = [];
 ...
 * @return number|null
 */
 // OJO FUNCTION DELICADA Y MUY USADA MEJOR NO TOPAR
...
Table.prototype.initializeRound = function( changeDealer ) {
	...
	if (!this.observers.length) {
		console.log('no humanos', this.public.name);
		this.stopGame();
		return;
	}
	...
	if( this.playersSittingInCount > 1 ) {
		...
		console.log('-----  nueva mano: ', 'headsUp', this.headsUp, 'playersInHandCount', this.playersInHandCount, 'button:', this.seats[this.public.dealerSeat].public.name);
		this.initializeSmallBlind();
	}
};

// se usa mas para test para reproducir el dealer y la mano exacta
Table.prototype.initializeRoundCustomDealer = function( nDealer ) {
	this.public.dealerSeat = nDealer;
...
Table.prototype.initializeSmallBlind = function() {
	...
	this.emitToPlayer(this.seats[this.public.activeSeat], 'postSmallBlind');
...
Table.prototype.initializePreflop = function() {
	...
	for( var i=0 ; i<this.playersInHandCount ; i++ ) {
		...
		this.emitToPlayer(this.seats[currentPlayer], 'dealingCards', this.seats[currentPlayer].cards );
...
Table.prototype.initializeNextPhase = function() {
	...
	this.public.biggestBet = 0;
									// NO HACER CAMBIOS AQUI AFECTA A TODO
	this.public.activeSeat = this.findNextPlayer( this.public.dealerSeat );
	...
	if( this.otherPlayersAreAllIn() ) {
		...
		this.emitToPlayer(this.seats[this.public.activeSeat], 'actNotBettedPot');
	}
};
...
Table.prototype.actionToNextPlayer = function() {
	...
	switch( this.public.phase ) {
		case 'smallBlind':
			...
			break;
		case 'bigBlind':
			this.emitToPlayer( this.seats[this.public.activeSeat], 'postBigBlind' );
			break;
		case 'preflop':
			if( this.otherPlayersAreAllIn() ) {
				if (this.isAcabaronTodos()){
					this.endPhase();
				}else{
					this.emitToPlayer( this.seats[this.public.activeSeat], 'actOthersAllIn' );
				}
			} else {
				this.emitToPlayer( this.seats[this.public.activeSeat], 'actBettedPot' );
			}
			break;
		case 'flop':
		case 'turn':
...
Table.prototype.playerFolded = function() {
	...
	console.log(this.getNameActive() + ' va foldear');
...
Table.prototype.playerCalled = function() {
	...
	console.log(this.getNameActive() + ' va pagar');
...
Table.prototype.playerBetted = function( amount ) {
	...
	console.log(this.getNameActive() + ' va apostar', amount);
...
Table.prototype.getOldBet = function() {
	return this.public.biggestBet;
}
...
Table.prototype.playerSatIn = function( seat ) {
	...
	if( this.autoBeginWhen2 && !this.gameIsOn && this.playersSittingInCount > 1 ) {
		// Initialize the game
		this.initializeRound( false );
	}
};
 ...
 * @param int 	seat 			(the number of the seat)
...
Table.prototype.otherPlayersAreAllIn = function() {
	...
	if(this.public.activeSeat == null){
		// revisa es un parche no tan validado
		return false;
	}
...
Table.prototype.endRound = function() {
	console.log('mano terminoooooooooooooooooooooooo');
	...
	for( i=0 ; i<this.public.seatsCount ; i++ ) {
		if( this.seats[i] !== null && this.seats[i].public.chipsInPlay <=0 && this.seats[i].public.sittingIn ) {
			if (this.autoRebuyBots && this.seats[i].isBot()) {
				console.log(this.seats[i].public.name, 'recomprooooooooooooooooooooooooooo');
				this.seats[i].public.chipsInPlay = 200;
			} else {
				this.seats[i].sitOut();
				this.playersSittingInCount--;
			}
 ...
 */
Table.prototype.getNameActive = function() {
	return this.seats[this.public.activeSeat].public.name;
}

Table.prototype.getTypeActive = function() {
	return this.seats[this.public.activeSeat].public.type;
}

Table.prototype.getBetActive = function() {
	return this.seats[this.public.activeSeat].public.bet;
}

Table.prototype.isTurnoBot = function() {
	return 	this.public.activeSeat != null &&
			!this.seats[this.public.activeSeat].socket &&
			this.seats[this.public.activeSeat].public.chipsInPlay > 0 &&
			!this.isAcabaronTodos();
}

// cuando ya nadie mas tiene que hablar porque ya estan allin
Table.prototype.isAcabaronTodos = function() {
	var acabaron = this.otherPlayersAreAllIn() && this.public.biggestBet === this.seats[this.public.activeSeat].public.bet;
	// console.log('acabaron todos:', acabaron, this.otherPlayersAreAllIn(), this.public.biggestBet, this.seats[this.public.activeSeat].public.bet, this.seats[this.public.activeSeat].public.name);
	return acabaron;
}

/**
 * tuve que isolar este proceso porque los bots no necesitan que se les envie
 * sabemos que es bot cuando player.socket es null,
 * adicionalmente es buen lugar para poner la logica automatica de los bots
 **/
Table.prototype.emitToPlayer = function(player, typeEmit, data){

	if(player === undefined){
		return;
	}
	// console.log('>>>> ', player.public.name, typeEmit, (player.socket ? player.socket.id : player.socket));
	...
	if(player.isBot()){
		switch (typeEmit){
			...
			case 'actOthersAllIn':
				break;
			default:
				console.log('FALLA RESPUESTA BOT NO CONSTA:' + typeEmit, player.public.name);
		}
		return;
	}

	if(this.autoBlinds && (typeEmit == 'postSmallBlind' || typeEmit == 'postBigBlind')){
		if(typeEmit == 'postSmallBlind'){this.playerPostedSmallBlind();return;}
		if(typeEmit == 'postBigBlind'){this.playerPostedBigBlind();return;}
	}

	if(data){
		player.socket.emit( typeEmit, data );
	}else{
		player.socket.emit( typeEmit );
	}

	// elejimos esta posicion porque debe ser luego de cada turno no depende de la velocidad de los bots
	// la ayuda en base a pruebas esta fallando
	//this.playRealIf(player, typeEmit);
}


//s!1563251388!129!25!50!6!0!Ah;Kh;!!0;5;2290;0!1;5;1094;25!2;5;2336;50!3;5;3119;0!4;7;965;0!5;5;1248;0!6;5;1403;0!7;5;2340;0!8;5;811;0/1;-1
//		     nt        bote   sb  bb  an    btn    propias     comuni            np   type fichas apuestas
//     "s!1517526111  !350!  100 !200!25    !4    !4d;8s   !   8s;3d;2c     !     3;   1;    9115;   3200
Table.prototype.getMessageToDrive = function(){
	// console.log('pots', this.pot.pots);
	var message = ['s',
		this.public.id,
		// el bote es impresiso pero drive no lo usa asi que no afecta
		this.pot.pots[0].amount,
		this.public.smallBlind,
		this.public.bigBlind,
		// no incluiremos antes
		'0',
		this.public.dealerSeat,
		// cartas del activo
		this.seats[this.public.activeSeat].cards[0] + ';' + this.seats[this.public.activeSeat].cards[1],
		//comunitarias
		getFixBoard(this.public.board)];

	// add players
	for( var i=0 ; i<this.public.seatsCount ; i++ ) {
		var player = this.seats[i];
		if( player && player.public.sittingIn && player.public.inHand ){
			// antes habia               && player.public.bet > 0 pero crea fallas cuando el allin se ve en las siguientes calles
			var chips = (player.public.chipsInPlay == 0) ? -200 : player.public.chipsInPlay;
			var mPlayer = i + ';' +
				player.public.typeN  + ';' +
				chips  + ';' +
				player.public.bet;

			message.push(mPlayer);
		}
	}

	// finalmente agregamos informacion de turno al message
	return message.join('!') + '/' + this.public.activeSeat + ';1';
};

// es la clase encargada de enviar las preguntas al server
// @player player|null si null es bot y auto ejecuta la accion, si vacio es player real y se le emite la ayuda.
Table.prototype.sendPost = function (value, controlado, player){

	var url = controlado == 'controlado' ?
			(urlBase + 'masterCashControlado/') :
			(urlBase + 'masterCash/');
	var table = this;
	request.post(
	    url,
...
// ejecuta la accion del bot si es turno
Table.prototype.playBotIf = function (){

	if(this.isTurnoBot()){
		var message = this.getMessageToDrive();
		if(this.lastMessage != message){
			var typeActive = this.getTypeActive();
			console.log('typeActive: ' + typeActive + ' message <' + message + '>');
			var controlado = typeActive == 'PRO' ? 'controlado' : 'noControlado';
			this.sendPost(message, controlado);
...
/**
 * Hace el pedido al server para mostrar la ayuda
 * @typeEmit string, evalua si esta pidiendo realmente la accion del player
 * @player player, el player al que se le va enviar la ayuda
 */
Table.prototype.playRealIf = function (player, typeEmit){

	if(typeEmit != 'postSmallBlind' && typeEmit != 'postBigBlind' && typeEmit != 'dealingCards'){
		var message = this.getMessageToDrive();
		if(this.lastMessage != message){
			console.log(typeEmit + ' turno real player: ' + player.public.name + ' <message <' + message + '>');
			this.sendPost(message, 'controlado', player);
			this.lastMessage = message;
		}
	}
};

function getFixBoard(arrayBoard){
	if(arrayBoard[0] == ''){
		return '';
	}
	if(arrayBoard[3] == ''){
		return arrayBoard[0] + ';' + arrayBoard[1] + ';' + arrayBoard[2];
	}
	if(arrayBoard[4] == ''){
		return arrayBoard[0] + ';' + arrayBoard[1] + ';' + arrayBoard[2] + ';' + arrayBoard[3];
	}
	return arrayBoard[0] + ';' + arrayBoard[1] + ';' + arrayBoard[2] + ';' + arrayBoard[3] + ';' + arrayBoard[4];
}

function executeRespuesta(table, respuesta){
	if(respuesta == 'fail'){
		return;
	}

    // toca foldear
    if (respuesta.indexOf('x') > -1 ||
    	respuesta.indexOf('y') > -1 ||
    	respuesta.indexOf('z') > -1 ){
			console.log('server: ' + respuesta, table.getNameActive() + ' va foldear');
			table.playerFolded();
			return;
    }

    // toca call
    if (respuesta.indexOf('a') > -1 ||
    	respuesta.indexOf('b') > -1 ||
    	respuesta.indexOf('c') > -1 ){

    	var isBigNoRaised = table.getOldBet() == table.getBetActive();
		if(table.getOldBet() > 0 && !isBigNoRaised){
			console.log('server: ' + respuesta, table.getNameActive() + ' va pagar');
			table.playerCalled();
		}else{
			console.log('server: ' + respuesta, table.getNameActive() + ' va checkear');
			table.playerChecked();
		}
		return;
    }
    ...
    // toca allin
    if (respuesta.indexOf('d') > -1 ||
    	respuesta.indexOf('m') > -1 ||
    	respuesta.indexOf('s') > -1 ){
			console.log('server: ' + respuesta, table.getNameActive() + ' va all in');
			table.playerRaised(table.seats[table.public.activeSeat].public.chipsInPlay);
			return;
    }

    // segun la posicion remplazara al 0 - 9
    var numeros = ['e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'n', 'o'];
	var apuestaStr = '';
	for(var i = 0; i < respuesta.length; i++){
		var char = respuesta.substring(i, i+1);
		apuestaStr += numeros.indexOf(char);
	}
	var apuesta = parseInt(apuestaStr);

	if(table.getOldBet() > 0){
		// cuando ya hay apuesta solo se suma lo que falta
		var lastBet = table.getBetActive();
		console.log('server: ' + respuesta, table.getNameActive() + ' va raise: ' + apuesta + ' - ' + lastBet);
		apuesta -= lastBet;
		table.playerRaised(apuesta);
	}else{
		console.log('server: ' + respuesta, table.getNameActive() + ' va bet: ' + apuesta);
		table.playerBetted(apuesta);
	}
}
